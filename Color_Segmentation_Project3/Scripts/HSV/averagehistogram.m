...
for k=1:numofsamples
    ...
    foreground_mask = double(maskR>thresh);
    ...
    foreground_mask = double(maskY>thresh);
    seg = I.* repmat(foreground_mask, [1,1,3]);
    %figure(2);imshow(seg);
    R = seg(:,:,1);
    G = seg(:,:,2);
    ...
    foreground_mask = double(maskG>thresh);
    seg = I.* repmat(foreground_mask, [1,1,3]);
    %figure(2);imshow(seg);
    R = seg(:,:,1);
    G = seg(:,:,2);
...
save('HSVSamples.mat','red_samples','yellow_samples','green_samples');